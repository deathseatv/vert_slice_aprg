function DomainGame(_eventbus, _procgen) constructor {
    ...
    domain.actions = {
		...
		step_simulation: function(_dt) {
		    ...
		    if (!d.move_active) return;
		    if (array_length(d.move_queue) <= 0) { d.move_active = false; return; }
		    ...
		    var _speed = 120; // pixels per second in orthographic world
		    var step = _speed * _dt;
		    ...
		    d.player.x += nx * min(step, dist);
		    d.player.y += ny * min(step, dist);
		},
